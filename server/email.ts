...
function buildNewTemplatesAnnouncementText(params: { name: string }): string {
  const firstName = params.name.split(" ")[0] || "there";
  return `
CONTRACTOR CIRCLE — 2 NEW TEMPLATES
...
export async function sendNewTemplatesAnnouncementEmail(params: {
  to: string;
  name: string;
}): Promise<{ success: boolean; id?: string; error?: string }> {
  if (!resend) {
