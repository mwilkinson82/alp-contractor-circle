...
// Fetch active members from the running server's API
async function getMembers() {
  // Use the server's tRPC endpoint to get active members
  const resp = await fetch("http://localhost:3000/api/trpc/members.listActive", {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  });
  
  if (!resp.ok) {
    // Fallback: query the database directly via drizzle
    console.log("tRPC endpoint not available, trying direct DB query...");
    return null;
  }
...
async function main() {
  console.log("Fetching active members list...");
  
  // Try to get members from the DB directly using the app's drizzle setup
  const { drizzle } = await import("drizzle-orm/mysql2");
  const mysql = await import("mysql2/promise");
  const { members } = await import("../drizzle/schema.ts");
