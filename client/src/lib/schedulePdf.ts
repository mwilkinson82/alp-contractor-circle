...
export async function generateSchedulePdf(options: PdfExportOptions): Promise<void> {
  ...
  function drawFooter(pageNum: number, totalPages: number) {
    ...
    if (footerConfig) {
      ...
      // Left
      renderFooterSlot(footerConfig.left, margin, footerMidY, "left");

      if (cols === 3) {
        renderFooterSlot(footerConfig.center, pageWidth / 2, footerMidY, "center");
        renderFooterSlot(footerConfig.right, pageWidth - margin, footerMidY, "right");
      } else if (cols === 4) {
      ...
      doc.text(footerText || "Generated by ALP CPM Schedule Builder", margin, defaultMidY);
      doc.text(`Page ${pageNum} of ${totalPages}`, pageWidth / 2, defaultMidY, { align: "center" });
      doc.text("\u00A9 ConstructLine", pageWidth - margin, defaultMidY, { align: "right" });
    }
  }
