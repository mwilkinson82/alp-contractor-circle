        ...
        <div className="overflow-y-auto overscroll-contain min-h-0 space-y-5 pr-1">
          ...
          <section className="space-y-2">
            ...
            <Textarea
              ...
              className="bg-white/5 border-white/10 text-cream placeholder:text-cream-muted/40 min-h-[140px] resize-none text-sm"
              maxLength={2000}
            />
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-cream-muted/50">{scopeText.length}/2000</span>
              {scopeText.trim() && (
                <button onClick={() => setScopeText("")} className="text-[10px] text-cream-muted hover:text-red-400 transition-colors">
                  Clear
                </button>
              )}
            </div>
            {scopeIntent.hasScope && (
              <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 space-y-2 overflow-hidden">
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  <span className="text-xs font-semibold text-amber-300 shrink-0">Interpreted Scope</span>
                  <span className="text-[10px] text-amber-300 bg-amber-500/15 border border-amber-500/25 rounded px-1.5 py-0.5 break-words max-w-full">
                    {scopeIntent.summary}
                  </span>
                </div>
                <p className="text-xs text-cream-muted break-words">
              ...
              <div className="flex flex-wrap gap-1.5 overflow-hidden">
                {SCOPE_EXAMPLES.map((example) => (
                  <button
                    ...
                    className="text-[11px] px-2 py-1 rounded-md bg-white/5 border border-white/10 text-cream-muted hover:bg-amber-500/10 hover:text-amber-300 hover:border-amber-500/20 transition-colors text-left break-words whitespace-normal max-w-full"
                  >
                    {example.length > 80 ? example.slice(0, 77) + "…" : example}
                  </button>
                ))}
              </div>
            </div>
          ...
          {/* --- Optional Takeoff Guidance (collapsed) --- */}
          <details className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
            <summary className="cursor-pointer text-sm font-semibold text-cream flex items-center gap-2">
              <ChevronDown className="w-4 h-4 text-cream-muted transition-transform" />
              Optional takeoff guidance
              <span className="text-[10px] font-normal text-cream-muted ml-1">CSI divisions &amp; trade specialties</span>
            </summary>
          ...
          {/* --- Optional Bid Adders (collapsed) --- */}
          <details className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
            <summary className="cursor-pointer text-sm font-semibold text-cream flex items-center gap-2">
              <ChevronDown className="w-4 h-4 text-cream-muted transition-transform" />
              Optional bid adders
              <span className="text-[10px] font-normal text-cream-muted ml-1">Allowances</span>
            </summary>
          ...
          {/* --- Setup Summary --- */}
          <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-xs overflow-hidden">
            ...
            <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-2 text-cream-muted">
              ...
              <span className="break-words">Scope: <strong className="text-cream">{scopeText.trim() ? scopeIntent.summary : "Full drawing set"}</strong></span>
              <span>Currency: <strong className="text-cream">{currency}</strong>{costRegion ? <> · Region: <strong className="text-cream">{costRegion}</strong></> : null}</span>
            </div>
          </div>
        </div>
