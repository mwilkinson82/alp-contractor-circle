...
export default function PreAnalysisModal({
  ...
  const [bidMode, setBidMode] = useState<TakeoffBidMode>(
    normalizeTakeoffBidMode(
      existingBidMode || saved.bidMode,
      DEFAULT_NEW_TAKEOFF_BID_MODE
    )
  );
  const [currency, setCurrency] = useState<"USD" | "GBP" | "AUD">(
    (existingCurrency as any) ||
  ...
  const allowancePresets = useMemo(
    () => getAllowancePresetsForProjectType(projectType),
    [projectType]
  );
  const tradePackageNeedsScope =
    bidMode === "trade_package" &&
    scopeText.trim().length < 12 &&
      ...
      <DialogContent className="sm:max-w-3xl max-h-[88vh] overflow-hidden flex flex-col border-[#d7c7aa] bg-[#f4efe4] text-[#171714] shadow-[0_32px_90px_rgba(41,37,28,0.34)] [&_[data-slot=dialog-header]]:border-[#d8c9ad] [&_[data-slot=dialog-footer]]:border-[#d8c9ad] [&_[data-slot=dialog-close]]:text-[#716855] [&_[data-slot=dialog-close]]:hover:bg-white [&_[data-slot=dialog-close]]:hover:text-[#171714]">
        <DialogHeader>
          ...
          <DialogDescription className="text-[#716855]">
            Pick your bid mode. ConstructLine builds the right review surface
            for {pendingSheetCount} drawing{pendingSheetCount !== 1 ? "s" : ""}.
          </DialogDescription>
        </DialogHeader>
          ...
          <section className="space-y-3">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <Label className="text-base font-semibold text-[#171714]">
                  Pick your bid mode
                </Label>
              </div>
              <p className="text-xs text-[#716855]">
                ConstructLine builds the right review surface for the bid you
                are building.
              </p>
                ...
                {TAKEOFF_BID_MODES.map(mode => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-1 text-[11px] leading-snug text-[#716855]">
                        {option.description}
                      </p>
                      <p className="mt-2 text-[10px] leading-snug text-[#8a6510]">
                        {option.reviewSurface}
                      </p>
                    </button>
                  );
                })}
            ...
            <div className="space-y-4 rounded-lg border border-[#d7c7aa] bg-white/70 p-3 shadow-sm">
              <div className="space-y-1">
                ...
                <p className="text-xs text-[#716855]">
                  Required for takeoff pricing. Pick the project type, currency,
                  and cost region before analysis starts.
                </p>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                  {TAKEOFF_PROJECT_TYPE_OPTIONS.map(option => (
                    ...
                    >
                      ...
                      <p className="mt-1 text-[10px] leading-snug text-[#716855]">
                        {option.description}
                      </p>
                    </button>
                  ))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    {CURRENCIES.map(c => (
                      ...
                      >
                        ...
                        <p className="text-[10px] text-[#716855] mt-1">
                          {c.description}
                        </p>
                      </button>
                    ))}
            ...
            {scopeIntent.hasScope && (
              <div className="rounded-lg border border-[#d7b44d] bg-[#fff7da] p-3 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs font-semibold text-[#8a6510]">
                    Review Surface
                  </span>
            ...
            {tradePackageNeedsScope && (
              <div className="rounded-lg border border-[#d7b44d] bg-[#fff4cb] p-3 text-xs text-[#8a6510]">
                Trade Package Takeoff works best with a scope boundary. Add a
                short scope, select divisions, or choose specialties so review
                and excluded items stay separated from the active bid.
              </div>
            )}
            <div className="space-y-1.5">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-[#8a806d]">
                Examples
              </span>
            ...
            <div className="rounded-lg border border-[#d7c7aa] bg-white/75 p-3 text-xs">
              ...
              <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-2 text-[#716855]">
                ...
                <span>
                  Currency:{" "}
                  <strong className="text-[#171714]">{currency}</strong>
                </span>
              </div>
            </div>
          </section>
          ...
          <details className="rounded-lg border border-[#d7c7aa] bg-white/65 p-3">
            <summary className="cursor-pointer text-sm font-semibold text-[#171714]">
              ...
              <span className="ml-2 text-xs font-normal text-[#716855]">
                Use these when you want to narrow sheet triage or trade
                boundaries.
              </span>
            </summary>
            <div className="space-y-5 pt-4">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {projectType === "other" && (
                    <p className="text-xs text-[#716855]">
                      Quick-add presets are hidden for Other / Not sure. Add
                      manual allowances as needed.
                    </p>
                  )}
                  {allowances.map(item => (
                    <div key={item.id} className="flex items-center gap-2">
                      ...
                      <div className="relative w-32">
                        <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#716855] text-sm">
                          $
                        </span>
                        <Input
                          type="number"
                      ...
                      <button
                        ...
                        className="text-orange-600/70 hover:text-orange-700 transition-colors p-1"
                      >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 text-sm text-[#8a6510] hover:text-[#6f4d00] transition-colors py-1"
                  >
