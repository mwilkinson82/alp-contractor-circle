import { Trash2, ChevronDown, ChevronRight, Palette, Pencil, Check, X } from "lucide-react";
...
interface WBSTreeProps {
  ...
  onUpdateNode?: (id: number, code: string, name: string) => void;
}
...
export function WBSTree({ nodes, onDelete, onUpdateColor, onUpdateNode }: WBSTreeProps) {
  ...
  const saveEdit = (node: WBSNode) => {
    if (onUpdateNode && editCode.trim() && editName.trim()) {
      onUpdateNode(node.id, editCode.trim(), editName.trim());
    }
