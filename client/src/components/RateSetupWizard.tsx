...
const PROJECT_SPECIALTIES = [
  {
    ...
    description: "Office buildings, retail, restaurants, tenant improvements",
    multiplier: 10000,
    icon: Building2,
  },
  {
    ...
    description:
      "Hospitals, clinics, medical offices — infection control, MEP-heavy",
    multiplier: 11500,
    icon: Shield,
  },
  {
    ...
    description: "Schools, universities, government buildings",
    multiplier: 10800,
    icon: Building2,
  },
  {
    ...
    description: "Warehouses, factories, distribution centers",
    multiplier: 10300,
    icon: Wrench,
  },
  {
    ...
    description:
      "Water/wastewater treatment, pump stations — specialized trades",
    multiplier: 12000,
    icon: Zap,
  },
  {
    id: "power_energy",
    label: "Power / Energy",
    description: "Power plants, substations, solar farms — high-voltage work",
    multiplier: 12500,
    icon: Zap,
  },
  {
    id: "high_rise",
    label: "High-Rise / Multi-Story",
    description:
      "High-rise residential or commercial — crane time, logistics premium",
    multiplier: 11200,
    icon: Building2,
  },
  {
    ...
    description:
      "Existing structure work — demolition, phasing, limited access",
    multiplier: 10500,
    icon: Wrench,
  },
];
...
const RES_SPECIALTIES = [
  {
    ...
    description: "Single-family homes, townhomes, duplexes",
    multiplier: 10000,
    icon: Home,
  },
  {
    id: "custom_home",
    label: "Custom / Luxury Home",
    description: "High-end finishes, complex architecture, premium materials",
    multiplier: 11500,
    icon: Home,
  },
  {
    id: "multi_family",
    label: "Multi-Family",
    description: "Apartments, condos, mixed-use residential",
    multiplier: 10800,
    icon: Building2,
  },
  {
    ...
    description: "Remodels, additions, kitchen/bath — existing structure work",
    multiplier: 10500,
    icon: Wrench,
  },
];
...
export default function RateSetupWizard({
  ...
  const deriveLaborType = (): LaborType => {
    if (workType === "residential")
      return shopType === "union" ? "res_union" : "res_open";
    return shopType === "union" ? "com_union" : "com_open";
  };

  const getSpecialtyMultiplier = (): number => {
    const list =
      workType === "commercial" ? PROJECT_SPECIALTIES : RES_SPECIALTIES;
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="rate-setup-wizard">
        <DialogHeader>
          ...
          <DialogDescription className="text-cream-muted">
            Set up your labor rates in under 60 seconds.{" "}
            <span className="font-semibold">
              <span className="text-white">Construct</span>
              <span className="text-amber-400">Line</span>
            </span>{" "}
            will calibrate 92 classifications across 20 trades.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="overflow-y-auto overscroll-contain min-h-0">
          ...
          <div className="flex items-center justify-center gap-1 py-3">
            {[1, 2, 3, 4, 5].map(s => (
              <div key={s} className="flex items-center">
                <button
                  onClick={() =>
                    s < step && step < 5 ? setStep(s) : undefined
                  }
                ...
                {s < 5 && (
                  <div
                    className={`w-8 h-0.5 transition-colors duration-300 ${s < step ? "bg-amber-500/40" : "bg-white/10"}`}
                  />
                )}
              </div>
            ))}
          ...
          <div className="flex items-center justify-center gap-1 pb-4">
            {["Work Type", "Shop Type", "Region", "Specialty", "Apply"].map(
              (label, i) => (
                <div key={label} className="flex items-center">
                  <span
                    className={`text-[9px] font-medium w-8 text-center ${
                      i + 1 === step
                        ? "text-amber-400"
                        : i + 1 < step
                          ? "text-amber-300/60"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <div className="flex flex-col items-center text-center gap-3">
                    ...
                    >
                      <Home
                        className={`w-8 h-8 ${workType === "residential" ? "text-amber-400" : "text-cream-muted"}`}
                      />
                    </div>
                    <div>
                      <h3
                        className={`text-lg font-bold ${workType === "residential" ? "text-amber-300" : "text-cream"}`}
                      >
                        Residential
                      </h3>
                      <p className="text-xs text-cream-muted mt-1">
                        Single-family, multi-family, custom homes, renovations
                      </p>
                    </div>
                  </div>
                  {workType === "residential" && (
                ...
                >
                  <div className="flex flex-col items-center text-center gap-3">
                    ...
                    >
                      <Building2
                        className={`w-8 h-8 ${workType === "commercial" ? "text-amber-400" : "text-cream-muted"}`}
                      />
                    </div>
                    <div>
                      <h3
                        className={`text-lg font-bold ${workType === "commercial" ? "text-amber-300" : "text-cream"}`}
                      >
                        Commercial
                      </h3>
                      <p className="text-xs text-cream-muted mt-1">
                        Office, retail, industrial, healthcare, institutional
                      </p>
                    </div>
                  </div>
                  {workType === "commercial" && (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <div className="flex flex-col items-center text-center gap-3">
                    ...
                    >
                      <Users
                        className={`w-8 h-8 ${shopType === "open" ? "text-amber-400" : "text-cream-muted"}`}
                      />
                    </div>
                    <div>
                      <h3
                        className={`text-lg font-bold ${shopType === "open" ? "text-amber-300" : "text-cream"}`}
                      >
                        Open Shop
                      </h3>
                      <p className="text-xs text-cream-muted mt-1">
                        Non-union labor. Flexible crew composition, market-rate
                        wages.
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-1 justify-center">
                      <Badge className="bg-white/10 text-cream-muted border-white/10 text-[9px]">
                        Lower burden
                      </Badge>
                      <Badge className="bg-white/10 text-cream-muted border-white/10 text-[9px]">
                        No fringe
                ...
                >
                  <div className="flex flex-col items-center text-center gap-3">
                    ...
                    >
                      <Shield
                        className={`w-8 h-8 ${shopType === "union" ? "text-amber-400" : "text-cream-muted"}`}
                      />
                    </div>
                    <div>
                      <h3
                        className={`text-lg font-bold ${shopType === "union" ? "text-amber-300" : "text-cream"}`}
                      >
                        Union
                      </h3>
                      <p className="text-xs text-cream-muted mt-1">
                        Union labor with prevailing wages, fringe benefits, and
                        training funds.
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-1 justify-center">
          ...
          {step === 3 && (
            <div className="space-y-4 py-2">
              ...
              <p className="text-xs text-cream-muted -mt-2">
                RS Means city cost indexes adjust your rates for local market
                conditions. Leave as National Average if you're not sure.
              </p>
              <RegionSelector
                value={regionCode}
                onChange={code => {
          ...
          {step === 4 && (
            <div className="space-y-4 py-2">
              ...
              <p className="text-xs text-cream-muted -mt-2">
                Different project types have different labor complexity. Select
                the closest match — or leave as Standard for no adjustment.
              </p>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        >
                          <Icon
                            className={`w-4.5 h-4.5 ${isSelected ? "text-amber-400" : "text-cream-muted"}`}
                          />
                        </div>
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-sm font-semibold ${isSelected ? "text-amber-300" : "text-cream"}`}
                            >
                              {s.label}
                            </span>
                          </div>
                          <p className="text-[10px] text-cream-muted mt-0.5 leading-tight">
                            {s.description}
                          </p>
                          {s.multiplier !== 10000 && (
              ...
              {!showComplete ? (
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-center gap-3 flex-wrap">
                    ...
                    {selectedSpecialty &&
                      selectedSpecialty.multiplier !== 10000 && (
                        <>
                          <span className="text-cream-muted">×</span>
                  ...
                  <div className="max-w-md mx-auto">
                    <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(processingPhase / PROCESSING_PHASES.length) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
