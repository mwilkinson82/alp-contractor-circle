        ...
        <main className="flex-1 p-4 sm:p-6 md:p-8 overflow-auto relative">
          {isBetaUser && !betaUser?.discordConnected && (
            <div className="mb-4 p-4 rounded-lg bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-between gap-4">
              ...
              <a
                href={`/api/beta/discord/connect?origin=${encodeURIComponent(window.location.origin)}&returnPath=${encodeURIComponent(window.location.pathname)}`}
                className="shrink-0 text-xs font-semibold text-indigo-300 border border-indigo-400/40 rounded-lg px-3 py-1.5 hover:bg-indigo-400/10 transition-all whitespace-nowrap"
              >
                Connect Discord
              </a>
            </div>
