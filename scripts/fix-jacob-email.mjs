...
const { data, error } = await supabase
  .from("members")
  .update({ email: "jake@nciconstruction.com" })
  .eq("email", "jae@nciconstruction.com")
  .select();

if (error) {
  console.error("❌ Supabase update failed:", error.message);
} else {
  console.log("✅ Supabase updated:", JSON.stringify(data, null, 2));
}
...
try {
  const { data: emailData, error: emailError } = await resend.emails.send({
    from: FROM,
    to: TO,
    subject: `${NAME}, welcome to The Contractor Circle`,
    html: welcomeHtml,
  ...
  if (emailError) {
    ...
    console.log(`✅ Welcome email sent to ${TO} — id: ${emailData?.id}`);
  }
} catch (err) {
