...
<body style="margin:0;padding:0;background:#0a0a0f;font-family:'Helvetica Neue',Arial,sans-serif;color:#f5f0e8;">
  <div style="max-width:600px;margin:0 auto;padding:40px 24px;">
    ...
    <div style="background:linear-gradient(135deg,rgba(196,120,62,0.08),transparent);border:1px solid rgba(196,120,62,0.2);border-radius:16px;padding:32px;margin-bottom:24px;">
      <h1 style="font-size:28px;font-weight:700;color:#f5f0e8;margin:0 0 12px;line-height:1.3;">⏰ Call Starts in 1 Hour</h1>
      
      <p style="font-size:16px;color:rgba(245,240,232,0.85);line-height:1.7;margin:0 0 24px;">Hey ${firstName} — just a heads up, tonight's Contractor Circle call starts in <strong style="color:#f5f0e8;">1 hour at 5 PM EST</strong>.</p>

      <p style="font-size:16px;color:rgba(245,240,232,0.85);line-height:1.7;margin:0 0 24px;">The Zoom link is in the portal. Log in now so you're ready to go.</p>
      ...
      <p style="font-size:16px;color:rgba(245,240,232,0.85);line-height:1.7;margin:24px 0 0;">See you in an hour.</p>
    </div>
  ...
  for (const member of members) {
    try {
      const { data, error } = await resend.emails.send({
        ...
        to: member.email,
        subject: "Call Starts in 1 Hour — See You at 5 PM",
...
console.log(`⏳ Waiting 15 minutes before sending (will fire at ${new Date(Date.now() + DELAY_MS).toLocaleTimeString()})...`);

setTimeout(() => {
  console.log(`\n🚀 15 minutes elapsed — sending now...`);
  sendAll();
}, DELAY_MS);
